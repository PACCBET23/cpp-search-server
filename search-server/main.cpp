...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        ...
        for (const auto word : stop_words_) {
            if (!IsValidWord(word)) {
                throw invalid_argument("Наличие недопустимых символов в стоп-словах"s);
            }
        }
    }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (text[0] == '-') {
            ...
            if (text[1] == '-') {
                throw invalid_argument("Наличие более чем одного минуса перед словами в поисковом запросе"s);
            }
            is_minus = true;
            text = text.substr(1);
        }
...
int main() {
    try {
        SearchServer search_server("и в н"s);
        ...
        cerr << search_server.GetDocumentId(-1) << endl;
    }
    catch (const invalid_argument& e) {
        cout << "Ошибка: "s << e.what() << endl;
    }
    catch (const out_of_range& i) {
        cout << "Ошибка: "s << i.what() << endl;
    }
}